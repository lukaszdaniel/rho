...
#else
	if (!Rf_isVector(value) || LENGTH(value) < 1) par_error(what);
#endif
...
/* else: cex can be a vector of length > 1, so pick off first value
   (as e.g. pch always did) */
	...
	const char *ss;
	value = Rf_coerceVector(value, STRSXP);
        if (STRING_ELT(value, 0) == NA_STRING) {
            SET_STRING_ELT(value, 0, R_BlankString);
        }
	lengthCheck(what, value, 1);
	const void *vmax = vmaxget();
	ss = Rf_translateChar(STRING_ELT(value, 0));
	if(strlen(ss) > 200)
	    Rf_error(_("graphical parameter 'family' has a maximum length of 200 bytes"));
#ifdef FOR_PAR
	...
	PROTECT(value = Rf_coerceVector(value, REALSXP));
	lengthCheck(what, value, 3);
	...
	if(REAL(value)[0] * REAL(value)[1] < 0 ||
	   REAL(value)[0] * REAL(value)[2] < 0)
	    Rf_warning("`mgp[1:3]' are of differing sign");
