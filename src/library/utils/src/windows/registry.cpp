...
static HKEY find_hive(const char *hkey)
{
    ...
    Rf_error(_("invalid '%s' value"),  "hive");
...
static SEXP readRegistryKey1(HKEY hkey, const wchar_t *name)
{
    ...
    case REG_BINARY:
	ans = Rf_allocVector(RAWSXP, size);
    ...
    case REG_LINK:
	Rf_warning("unhandled key type %s\n", "REG_LINK");
	ans = Rf_mkString("<REG_LINK>");
	break;
    case REG_RESOURCE_LIST:
	Rf_warning("unhandled key type %s\n", "REG_RESOURCE_LIST");
	ans = Rf_mkString("<REG_RESOURCE_LIST>");
	break;
    default:
	Rf_warning("unhandled key type %d\n", type);
    }
    return ans;
}
...
static SEXP readRegistryKey(HKEY hkey, int depth, int view)
{
    ...
    if (depth <= 0) return Rf_mkString("<subkey>");
	...
	Rf_error("RegQueryInfoKey error code %d: '%s'", (int) res,
	      formatError(res));
    size0 = max(maxsubkeylen, maxvalnamlen) + 1;
    name = (wchar_t *) R_alloc(size0, sizeof(wchar_t));
    PROTECT(ans = Rf_allocVector(VECSXP, nval + nsubkeys));
    PROTECT(nm = Rf_allocVector(STRSXP, nval+ nsubkeys));
    if (nval > 0) {
	PROTECT(ans0 = Rf_allocVector(VECSXP, nval));
	PROTECT(nm0 = Rf_allocVector(STRSXP, nval));
	for (i = 0; i < nval; i++) {
	    size = size0;
	...
	for (i = 0; i < nval; i++, k++) {
	    ...
	    if (LENGTH(tmp = STRING_ELT(nm0, indx[i])))
	    	...
	    	SET_STRING_ELT(nm, k, Rf_mkChar("(Default)"));
	}
	UNPROTECT(3);
    }
    if (nsubkeys > 0) {
	PROTECT(ans0 = Rf_allocVector(VECSXP, nsubkeys));
	PROTECT(nm0 = Rf_allocVector(STRSXP, nsubkeys));
...
SEXP readRegistry(SEXP call, SEXP op, SEXP args, SEXP env)
{
	...
	Rf_error(_("invalid '%s' value"),  "key");
    key = filenameToWchar(STRING_ELT(CAR(args), 0), 0);
    if(!Rf_isString(CADR(args)) || LENGTH(CADR(args)) != 1)
	Rf_error(_("invalid '%s' value"),  "hive");
    maxdepth = Rf_asInteger(CADDR(args));
    if(maxdepth == NA_INTEGER || maxdepth < 1)
	Rf_error(_("invalid '%s' value"),  "maxdepth");
    hive = find_hive(R_CHAR(STRING_ELT(CADR(args), 0)));
    view = Rf_asInteger(CADDDR(args));
    ...
    */
	...
	Rf_error(_("Registry key '%ls' not found"), key);
    if (res != ERROR_SUCCESS)
	Rf_error("RegOpenKeyEx error code %d: '%s'", (int) res, formatError(res));
