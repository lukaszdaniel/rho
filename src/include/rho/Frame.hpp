...
namespace rho
{
  ...
  private:
    ...
    Frame(const ArgList &promised_args, size_t size = 16, bool check_list_size = true);
    Frame(const FrameDescriptor *descriptor, const ArgList &promised_args);

  public:
    static Frame *closureWorkingFrame(const FrameDescriptor *descriptor, const ArgList &promised_args)
    {
      return new Frame(descriptor, promised_args);
    }
    static Frame *closureWorkingFrame(const ArgList &promised_args, size_t size = 16, bool check_list_size = true)
    {
      return new Frame(promised_args, size, check_list_size);
    }
    static Frame *normalFrame(size_t size = 16, bool check_list_size = true)
    {
    ...
    Binding *obtainBinding(const Symbol *symbol, int location)
    {
      ...
      if (!binding->isSet())
      {
        ...
        m_used_bindings_size = std::max(m_used_bindings_size, (unsigned char)(location + 1));
      }
      return binding;
    }
