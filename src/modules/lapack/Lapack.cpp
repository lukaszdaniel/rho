...
static char La_norm_type(const char *typstr)
{
	...
	Rf_error(_("argument type[1]='%s' must be one of 'M','1','O','I','F' or 'E'"),
	      typstr);
    return typup;
}
...
static char La_rcond_type(const char *typstr)
{
    ...
    if (strlen(typstr) != 1)
	Rf_error(_("argument type[1]='%s' must be a character string of string length 1"),
	      typstr);
    typup = (char) toupper(*typstr);
    if (typup == '1')
	typup = 'O'; /* alias */
    else if (typup != 'O' && typup != 'I')
	Rf_error(_("argument type[1]='%s' must be one of '1','O', or 'I'"),
	      typstr);
    return typup; /* 'O' or 'I' */
}
...
{
    if (!Rf_isString(jobu))
	Rf_error("'jobu' must be a character string");
    int *xdims = INTEGER(Rf_coerceVector(Rf_getAttrib(x, R_DimSymbol), INTSXP)),
	n = xdims[0], p = xdims[1], nprot = 2;

    /* work on a copy of x  */
    double *xvals;
    if (!Rf_isReal(x)) {
	x = PROTECT(Rf_coerceVector(x, REALSXP)); nprot++;
	xvals = REAL(x);
    } else {
    ...
    SEXP dims = Rf_getAttrib(u, R_DimSymbol);
    if (TYPEOF(dims) != INTSXP) Rf_error("non-integer dim(u)");
    int ldu = INTEGER(dims)[0];
    dims = Rf_getAttrib(vt, R_DimSymbol);
    if (TYPEOF(dims) != INTSXP) Rf_error("non-integer dim(vt)");
    ...
    const char *ju = R_CHAR(STRING_ELT(jobu, 0));
    int info = 0, lwork = -1;
    F77_CALL(dgesdd)(ju, &n, &p, xvals, &n, REAL(s),
		     REAL(u), &ldu, REAL(vt), &ldvt,
		     &tmp, &lwork, iwork, &info);
    if (info != 0)
	Rf_error(_("error code %d from Lapack routine '%s'"), info, "dgesdd");
    lwork = (int) tmp;
    ...
    SET_STRING_ELT(nm, 0, Rf_mkChar("d"));
    SET_STRING_ELT(nm, 1, Rf_mkChar("u"));
    SET_STRING_ELT(nm, 2, Rf_mkChar("vt"));
    Rf_setAttrib(val, R_NamesSymbol, nm);
...
static SEXP La_rs(SEXP x, SEXP only_values)
{
    ...
    n = xdims[0];
    if (n != xdims[1]) Rf_error(_("'x' must be a square numeric matrix"));
    ov = Rf_asLogical(only_values);
    if (ov == NA_LOGICAL) Rf_error(_("invalid '%s' argument"), "only.values");
    ...
    if (info != 0)
	Rf_error(_("error code %d from Lapack routine '%s'"), info, "dsyevr");
    lwork = (int) tmp;
    liwork = itmp;
    ...
    SEXP ret, nm;
    if (!ov) {
	ret = PROTECT(Rf_allocVector(VECSXP, 2));
	nm = PROTECT(Rf_allocVector(STRSXP, 2));
	SET_STRING_ELT(nm, 1, Rf_mkChar("vectors"));
	SET_VECTOR_ELT(ret, 1, z);
	UNPROTECT_PTR(z);
    } else {
	ret = PROTECT(Rf_allocVector(VECSXP, 1));
	nm = PROTECT(Rf_allocVector(STRSXP, 1));
    }
...
{
    int i, j;
    SEXP s = Rf_allocMatrix(CPLXSXP, n, n);
    size_t N = n;
    for (j = 0; j < n; j++) {
	if (imaginary[j] != 0) {
	    int j1 = j + 1;
...
static SEXP La_rg(SEXP x, SEXP only_values)
{
    ...
    n = xdims[0];
    if (n != xdims[1])
	Rf_error(_("'x' must be a square numeric matrix"));
    ...
    if (info != 0)
	Rf_error(_("error code %d from Lapack routine '%s'"), info, "dgeev");
    lwork = (int) tmp;
    work = (double *) R_alloc(lwork, sizeof(double));
    F77_CALL(dgeev)(jobVL, jobVR, &n, xvals, &n, wR, wI,
	...
	SET_VECTOR_ELT(ret, 0, val);
	if(vectors) {
	    val = Rf_allocMatrix(REALSXP, n, n);
...
static SEXP La_dlange(SEXP A, SEXP type)
{
    ...
    if (!Rf_isMatrix(A)) Rf_error(_("'A' must be a numeric matrix"));
    if (!Rf_isString(type))
	Rf_error(_("'type' must be a character string"));
    if (!Rf_isReal(A)) {
	A = PROTECT(Rf_coerceVector(A, REALSXP)); nprot++;
    }

    xdims = INTEGER(Rf_coerceVector(Rf_getAttrib(A, R_DimSymbol), INTSXP));
    m = xdims[0];
    n = xdims[1]; /* m x n  matrix {using Lapack naming convention} */

    typNorm[0] = La_norm_type(R_CHAR(Rf_asChar(type)));

    SEXP val = PROTECT(Rf_allocVector(REALSXP, 1));
...
static SEXP La_dgecon(SEXP A, SEXP norm)
{
    ...
    if (!Rf_isMatrix(A)) Rf_error(_("'A' must be a numeric matrix"));
    if (!Rf_isString(norm))
	Rf_error(_("'norm' must be a character string"));
    A = PROTECT(Rf_isReal(A) ? Rf_duplicate(A) : Rf_coerceVector(A, REALSXP));

    xdims = INTEGER(Rf_coerceVector(Rf_getAttrib(A, R_DimSymbol), INTSXP));
    m = xdims[0]; n = xdims[1];

    typNorm[0] = La_rcond_type(R_CHAR(Rf_asChar(norm)));
	...
	if (info < 0) {
	    UNPROTECT(2);
	    Rf_error(_("error [%d] from Lapack 'dgetrf()'"), info);
	}
	else { /* i := info > 0:  LU decomp. is completed, but  U[i,i] = 0
		* <==> singularity */
#if 0
	    Rf_warning(_("exact singularity: U[%d,%d] = 0 in LU-decomposition {Lapack 'dgetrf()'}"),
		    info,info);
#endif
    ...
    F77_CALL(dgecon)(typNorm, &n, REAL(A), &n, &anorm,
		     ...
		     work, iwork, &info);
    UNPROTECT(2);
    if (info) Rf_error(_("error [%d] from Lapack 'dgecon()'"), info);
    return val;
}
...
static SEXP La_dtrcon(SEXP A, SEXP norm)
{
    ...
    if (!Rf_isMatrix(A)) Rf_error(_("'A' must be a numeric matrix"));
    if (!Rf_isString(norm)) Rf_error(_("'norm' must be a character string"));
    if (!Rf_isReal(A)) {
	nprot++;
    ...
    if(n != xdims[1]) {
	UNPROTECT(nprot);
	Rf_error(_("'A' must be a *square* matrix"));
    }
    ...
    F77_CALL(dtrcon)(typNorm, "U", "N", &n, REAL(A), &n,
		     ...
		     &info);
    UNPROTECT(nprot);
    if (info) Rf_error(_("error [%d] from Lapack 'dtrcon()'"), info);
    return val;
}
...
static SEXP La_zgecon(SEXP A, SEXP norm)
{
    ...
    if (!Rf_isString(norm)) Rf_error(_("'norm' must be a character string"));
    if (!(Rf_isMatrix(A) && Rf_isComplex(A)))
	Rf_error(_("'A' must be a complex matrix"));
    dims = INTEGER(Rf_coerceVector(Rf_getAttrib(A, R_DimSymbol), INTSXP));
    n = dims[0];
    if(n != dims[1]) Rf_error(_("'A' must be a *square* matrix"));
	...
	if (info < 0) {
	    UNPROTECT(1);
	    Rf_error(_("error [%d] from Lapack 'zgetrf()'"), info);
	} else {
	    REAL(val)[0] = 0.; /* rcond = 0 <==> singularity */
	    UNPROTECT(1);
	    return val;
    ...
    F77_CALL(zgecon)(typNorm, &n, avals, &n, &anorm,
		     ...
		     rwork, &info);
    UNPROTECT(1);
    if (info) Rf_error(_("error [%d] from Lapack 'zgecon()'"), info);
    return val;
...
static SEXP La_ztrcon(SEXP A, SEXP norm)
{
    ...
    val = PROTECT(Rf_allocVector(REALSXP, 1));
    ...
    F77_CALL(ztrcon)(typNorm, "U", "N", &n, COMPLEX(A), &n,
		     ...
		     &info);
    UNPROTECT(1);
    if (info) Rf_error(_("error [%d] from Lapack 'ztrcon()'"), info);
    return val;
#else
    Rf_error(_("Fortran complex functions are not available on this platform"));
    return nullptr; /* -Wall */
...
static SEXP La_solve_cmplx(SEXP A, SEXP Bin)
{
    ...
    if (!Rf_isMatrix(A)) Rf_error(_("'a' must be a complex matrix"));
    Adims = INTEGER(Rf_coerceVector(Rf_getAttrib(A, R_DimSymbol), INTSXP));
    n = Adims[0];
    if(n == 0) Rf_error(_("'a' is 0-diml"));
    int n2 = Adims[1];
    if(n2 != n) Rf_error(_("'a' (%d x %d) must be square"), n, n2);
    Adn = Rf_getAttrib(A, R_DimNamesSymbol);

    if (Rf_isMatrix(Bin)) {
	Bdims = INTEGER(Rf_coerceVector(Rf_getAttrib(Bin, R_DimSymbol), INTSXP));
	p = Bdims[1];
	if(p == 0) Rf_error(_("no right-hand side in 'b'"));
	int p2 = Bdims[0];
	if(p2 != n)
	    Rf_error(_("'b' (%d x %d) must be compatible with 'a' (%d x %d)"),
		  p2, p, n, n);
	PROTECT(B = Rf_allocMatrix(CPLXSXP, n, p));
	SEXP Bindn =  Rf_getAttrib(Bin, R_DimNamesSymbol);
	if (!Rf_isNull(Adn) || !Rf_isNull(Bindn)) {
	    Bdn = Rf_allocVector(VECSXP, 2);
	    if (!Rf_isNull(Adn)) SET_VECTOR_ELT(Bdn, 0, VECTOR_ELT(Adn, 1));
	    if (!Rf_isNull(Bindn)) SET_VECTOR_ELT(Bdn, 1, VECTOR_ELT(Bindn, 1));
	    if (!Rf_isNull(VECTOR_ELT(Bdn, 0)) && !Rf_isNull(VECTOR_ELT(Bdn, 1)))
		Rf_setAttrib(B, R_DimNamesSymbol, Bdn);
	}
    } else {
	p = 1;
	if(Rf_length(Bin) != n)
	    Rf_error(_("'b' (%d x %d) must be compatible with 'a' (%d x %d)"),
		  Rf_length(Bin), p, n, n);
	PROTECT(B = Rf_allocVector(CPLXSXP, n));
	if (!Rf_isNull(Adn)) Rf_setAttrib(B, R_NamesSymbol, VECTOR_ELT(Adn, 1));
    }
    ...
    if (info < 0)
	Rf_error(_("argument %d of Lapack routine %s had invalid value"),
	      -info, "zgesv");
    if (info > 0)
	Rf_error(("Lapack routine zgesv: system is exactly singular"));
    UNPROTECT(3);  /* B, Bin, A */
    return B;
#else
    Rf_error(_("Fortran complex functions are not available on this platform"));
...
static SEXP La_qr_cmplx(SEXP Ain)
{
    ...
    if (!(Rf_isMatrix(Ain) && Rf_isComplex(Ain)))
	Rf_error(_("'a' must be a complex matrix"));
    SEXP Adn = Rf_getAttrib(Ain, R_DimNamesSymbol);
    Adims = INTEGER(Rf_coerceVector(Rf_getAttrib(Ain, R_DimSymbol), INTSXP));
    m = Adims[0]; n = Adims[1];
    SEXP A = PROTECT(Rf_allocMatrix(CPLXSXP, m, n));
    Memcpy(COMPLEX(A), COMPLEX(Ain), (size_t)m * n);
    rwork = (double *) R_alloc(2*(size_t)n, sizeof(double));

    SEXP jpvt = PROTECT(Rf_allocVector(INTSXP, n));
    for (i = 0; i < n; i++) INTEGER(jpvt)[i] = 0;
    SEXP tau = PROTECT(Rf_allocVector(CPLXSXP, m < n ? m : n));
    lwork = -1;
    F77_CALL(zgeqp3)(&m, &n, COMPLEX(A), &m, INTEGER(jpvt), COMPLEX(tau),
		     &tmp, &lwork, rwork, &info);
    if (info != 0)
	Rf_error(_("error code %d from Lapack routine '%s'"), info, "zgeqp3");
    lwork = (int) tmp.r;
    ...
    if (info != 0)
	Rf_error(_("error code %d from Lapack routine '%s'"), info, "zgeqp3");
    SEXP val = PROTECT(Rf_allocVector(VECSXP, 4));
    SEXP nm = PROTECT(Rf_allocVector(STRSXP, 4));
    SET_STRING_ELT(nm, 0, Rf_mkChar("qr"));
    SET_STRING_ELT(nm, 1, Rf_mkChar("rank"));
    SET_STRING_ELT(nm, 2, Rf_mkChar("qraux"));
    SET_STRING_ELT(nm, 3, Rf_mkChar("pivot"));
    Rf_setAttrib(val, R_NamesSymbol, nm);
    // Fix up dimnames(A)
    if(!Rf_isNull(Adn)) {
	SEXP Adn2 = Rf_duplicate(Adn);
	SEXP cn = VECTOR_ELT(Adn, 1), cn2 = VECTOR_ELT(Adn2, 1);
	if(!Rf_isNull(cn)) { // pivot them
	    for (int j = 0; j < n; j++)
		SET_STRING_ELT(cn2, j, STRING_ELT(cn, INTEGER(jpvt)[j]-1));
	}
	Rf_setAttrib(A, R_DimNamesSymbol, Adn2);
    }
    SET_VECTOR_ELT(val, 0, A);
    SET_VECTOR_ELT(val, 1, Rf_ScalarInteger(m < n ? m : n));
...
static SEXP qr_coef_cmplx(SEXP Q, SEXP Bin)
{
    ...
    k = LENGTH(tau);
    if (!Rf_isMatrix(Bin)) Rf_error(_("'b' must be a complex matrix"));

    if (!Rf_isComplex(Bin)) B = PROTECT(Rf_coerceVector(Bin, CPLXSXP));
    else B = PROTECT(Rf_duplicate(Bin));

    n = INTEGER(Rf_coerceVector(Rf_getAttrib(qr, R_DimSymbol), INTSXP))[0];
    Bdims = INTEGER(Rf_coerceVector(Rf_getAttrib(Bin, R_DimSymbol), INTSXP));
    if(Bdims[0] != n)
	Rf_error(_("right-hand side should have %d not %d rows"), n, Bdims[0]);
    nrhs = Bdims[1];
    lwork = -1;
    F77_CALL(zunmqr)("L", "C", &n, &nrhs, &k,
		     ...
		     &tmp, &lwork, &info);
    if (info != 0)
	Rf_error(_("error code %d from Lapack routine '%s'"), info, "zunmqr");
    lwork = (int) tmp.r;
    work = (Rcomplex *) R_alloc(lwork, sizeof(Rcomplex));
    F77_CALL(zunmqr)("L", "C", &n, &nrhs, &k,
		     COMPLEX(qr), &n, COMPLEX(tau), COMPLEX(B), &n,
    ...
    if (info != 0)
	Rf_error(_("error code %d from Lapack routine '%s'"), info, "ztrtrs");
    UNPROTECT(1);
    return B;
#else
    Rf_error(_("Fortran complex functions are not available on this platform"));
...
static SEXP qr_qy_cmplx(SEXP Q, SEXP Bin, SEXP trans)
{
    ...
    k = LENGTH(tau);
    if (!(Rf_isMatrix(Bin) && Rf_isComplex(Bin)))
	Rf_error(_("'b' must be a complex matrix"));
    tr = Rf_asLogical(trans);
    if(tr == NA_LOGICAL) Rf_error(_("invalid '%s' argument"), "trans");

    if (!Rf_isReal(Bin)) B = PROTECT(Rf_coerceVector(Bin, CPLXSXP));
    else B = PROTECT(Rf_duplicate(Bin));
    n = INTEGER(Rf_coerceVector(Rf_getAttrib(qr, R_DimSymbol), INTSXP))[0];
    Bdims = INTEGER(Rf_coerceVector(Rf_getAttrib(B, R_DimSymbol), INTSXP));
    if(Bdims[0] != n)
	Rf_error(_("right-hand side should have %d not %d rows"), n, Bdims[0]);
    nrhs = Bdims[1];
    lwork = -1;
...
static SEXP La_svd_cmplx(SEXP jobu, SEXP x, SEXP s, SEXP u, SEXP v)
{
#ifdef HAVE_FORTRAN_DOUBLE_COMPLEX
    if (!(Rf_isString(jobu)))
	Rf_error(_("'jobu' must be a character string"));
    int *xdims = INTEGER(Rf_coerceVector(Rf_getAttrib(x, R_DimSymbol), INTSXP));
    int n = xdims[0], p = xdims[1];
    const char *jz = R_CHAR(STRING_ELT(jobu, 0));

    /* The underlying LAPACK, specifically ZLARF, does not work with
     * long arrays */
    if ((double)n * (double)p > INT_MAX)
	Rf_error(_("matrices of 2^31 or more elements are not supported"));
    ...
    int ldu, ldv;
    SEXP dims = Rf_getAttrib(u, R_DimSymbol);
    if (TYPEOF(dims) != INTSXP) Rf_error("non-integer dims");
    ldu = INTEGER(dims)[0];
    dims = Rf_getAttrib(v, R_DimSymbol);
    if (TYPEOF(dims) != INTSXP) Rf_error("non-integer dims");
    ldv = INTEGER(dims)[0];
    ...
    if (info != 0)
	Rf_error(_("error code %d from Lapack routine '%s'"), info, "zgesdd");
    lwork = (int) tmp.r;
...
static SEXP La_rs_cmplx(SEXP xin, SEXP only_values)
{
    ...
    xdims = INTEGER(Rf_coerceVector(Rf_getAttrib(xin, R_DimSymbol), INTSXP));
    n = xdims[0];
    if (n != xdims[1]) Rf_error(_("'x' must be a square complex matrix"));
    ov = Rf_asLogical(only_values);
    if (ov == NA_LOGICAL) Rf_error(_("invalid '%s' argument"), "only.values");
    if (ov) jobv[0] = 'N'; else jobv[0] = 'V';

    SEXP x = PROTECT(Rf_allocMatrix(CPLXSXP, n, n));
    rx = COMPLEX(x);
    ...
    F77_CALL(zheev)(jobv, uplo, &n, rx, &n, rvalues, &tmp, &lwork, rwork,
		    &info);
    if (info != 0)
	Rf_error(_("error code %d from Lapack routine '%s'"), info, "zheev");
    lwork = (int) tmp.r;
    work = (Rcomplex *) R_alloc(lwork, sizeof(Rcomplex));
...
static SEXP La_rg_cmplx(SEXP x, SEXP only_values)
{
    ...
    if (!ov) {
	jobVR[0] = 'V';
	PROTECT(val = Rf_allocMatrix(CPLXSXP, n, n));
	right = COMPLEX(val);
    }
    PROTECT(values = Rf_allocVector(CPLXSXP, n));
    rwork = (double *) R_alloc(2*(size_t)n, sizeof(double));
    /* ask for optimal size of work array */
    lwork = -1;
    ...
    if (info != 0)
	Rf_error(_("error code %d from Lapack routine '%s'"), info, "zgeev");
    lwork = (int) tmp.r;
    work = (Rcomplex *) R_alloc(lwork, sizeof(Rcomplex));
    F77_CALL(zgeev)(jobVL, jobVR, &n, xvals, &n, COMPLEX(values),
...
static SEXP La_chol(SEXP A, SEXP pivot, SEXP stol)
{
    ...
    SEXP ans = PROTECT(Rf_isReal(A) ? Rf_duplicate(A): Rf_coerceVector(A, REALSXP));
    SEXP adims = Rf_getAttrib(A, R_DimSymbol);
    if (TYPEOF(adims) != INTSXP) Rf_error("non-integer dims");
    int m = INTEGER(adims)[0], n = INTEGER(adims)[1];

    if (m != n) Rf_error(_("'a' must be a square matrix"));
    if (m <= 0) Rf_error(_("'a' must have dims > 0"));
    size_t N = n;
    for (int j = 0; j < n; j++) 	/* zero the lower triangle */
	for (int i = j+1; i < n; i++) REAL(ans)[i + N * j] = 0.;

    int piv = Rf_asInteger(pivot);
    if (piv != 0 && piv != 1) Rf_error("invalid '%s' value", "pivot");
    if(!piv) {
	int info;
	F77_CALL(dpotrf)("Upper", &m, REAL(ans), &m, &info);
	if (info != 0) {
	    if (info > 0)
		Rf_error(_("the leading minor of order %d is not positive definite"),
		      info);
	    Rf_error(_("argument %d of Lapack routine %s had invalid value"),
		  -info, "dpotrf");
	}
	...
	SEXP piv = PROTECT(Rf_allocVector(INTSXP, m));
	int *ip = INTEGER(piv);
	...
	if (info != 0) {
	    if (info > 0)
		Rf_warning(_("the matrix is either rank-deficient or indefinite"));
	    else
		Rf_error(_("argument %d of Lapack routine %s had invalid value"),
		      -info, "dpstrf");
	}
	Rf_setAttrib(ans, Rf_install("pivot"), piv);
	SEXP s_rank = Rf_install("rank");
	Rf_setAttrib(ans, s_rank, Rf_ScalarInteger(rank));
	SEXP cn, dn = Rf_getAttrib(ans, R_DimNamesSymbol);
	if (!Rf_isNull(dn) && !Rf_isNull(cn = VECTOR_ELT(dn, 1))) {
	    // need to pivot the colnames
	    SEXP dn2 = PROTECT(Rf_duplicate(dn));
	    SEXP cn2 = VECTOR_ELT(dn2, 1);
	    for(int i = 0; i < m; i++)
		SET_STRING_ELT(cn2, i, STRING_ELT(cn, ip[i] - 1)); // base 1
	    Rf_setAttrib(ans, R_DimNamesSymbol, dn2);
	    UNPROTECT(1);
	}
	UNPROTECT(1); // piv
    }
...
static SEXP La_chol2inv(SEXP A, SEXP size)
{
	...
	if (sz == 1 && !Rf_isMatrix(A) && Rf_isReal(A)) {
	    /* nothing to do; m = n = 1; ... */
	} else if (Rf_isMatrix(A)) {
	    SEXP adims = Rf_getAttrib(A, R_DimSymbol);
	    if (TYPEOF(adims) != INTSXP) Rf_error("non-integer dims");
	    Amat = PROTECT(Rf_coerceVector(A, REALSXP)); nprot++;
	    m = INTEGER(adims)[0]; n = INTEGER(adims)[1];
	} else Rf_error(_("'a' must be a numeric matrix"));

	if (sz > n) { UNPROTECT(nprot); Rf_error(_("'size' cannot exceed ncol(x) = %d"), n); }
	if (sz > m) { UNPROTECT(nprot); Rf_error(_("'size' cannot exceed nrow(x) = %d"), m); }
	ans = PROTECT(Rf_allocMatrix(REALSXP, sz, sz)); nprot++;
	size_t M = m, SZ = sz;
	for (int j = 0; j < sz; j++) {
	    for (int i = 0; i <= j; i++)
	...
	if (info != 0) {
	    UNPROTECT(nprot);
	    if (info > 0)
		Rf_error(_("element (%d, %d) is zero, so the inverse cannot be computed"),
		      info, info);
	    Rf_error(_("argument %d of Lapack routine %s had invalid value"),
		  -info, "dpotri");
	}
...
{
    int n, p;
    double *avals, anorm, rcond, tol = Rf_asReal(tolin), *work;
    SEXP B, Adn, Bdn;

    if (!(Rf_isMatrix(A) &&
	  (TYPEOF(A) == REALSXP || TYPEOF(A) == INTSXP || TYPEOF(A) == LGLSXP)))
	Rf_error(_("'a' must be a numeric matrix"));
    int *Adims = INTEGER(Rf_coerceVector(Rf_getAttrib(A, R_DimSymbol), INTSXP));
    n = Adims[0];
    if(n == 0) Rf_error(_("'a' is 0-diml"));
    int n2 = Adims[1];
    if(n2 != n) Rf_error(_("'a' (%d x %d) must be square"), n, n2);
    ...
    if (Rf_isMatrix(Bin)) {
	int *Bdims = INTEGER(Rf_coerceVector(Rf_getAttrib(Bin, R_DimSymbol), INTSXP));
	p = Bdims[1];
	if(p == 0) Rf_error(_("no right-hand side in 'b'"));
	int p2 = Bdims[0];
	if(p2 != n)
	    Rf_error(_("'b' (%d x %d) must be compatible with 'a' (%d x %d)"),
		  p2, p, n, n);
	PROTECT(B = Rf_allocMatrix(REALSXP, n, p));
	SEXP Bindn =  Rf_getAttrib(Bin, R_DimNamesSymbol);
	...
	if(Rf_length(Bin) != n)
	    Rf_error(_("'b' (%d x %d) must be compatible with 'a' (%d x %d)"),
		  Rf_length(Bin), p, n, n);
	PROTECT(B = Rf_allocVector(REALSXP, n));
	if (!Rf_isNull(Adn)) Rf_setAttrib(B, R_NamesSymbol, VECTOR_ELT(Adn, 1));
    }
    PROTECT(Bin = Rf_coerceVector(Bin, REALSXP));
	...
	Rf_error(_("argument %d of Lapack routine %s had invalid value"),
	      -info, "dgesv");
    if (info > 0)
	Rf_error(_("Lapack routine %s: system is exactly singular: U[%d,%d] = 0"),
	      "dgesv", info, info);
    if(tol > 0) {
	char one[2] = "1";
	...
	if (rcond < tol)
	    Rf_error(_("system is computationally singular: reciprocal condition number = %g"),
		  rcond);
    }
    UNPROTECT(3); /* B, Bin, A */
    return B;
...
static SEXP La_qr(SEXP Ain)
{
    ...
    if (!Rf_isMatrix(Ain)) Rf_error(_("'a' must be a numeric matrix"));
    SEXP Adn = Rf_getAttrib(Ain, R_DimNamesSymbol);
    int *Adims = INTEGER(Rf_coerceVector(Rf_getAttrib(Ain, R_DimSymbol), INTSXP));
    m = Adims[0]; n = Adims[1];
    SEXP A;
    if (!Rf_isReal(Ain)) {
	A = PROTECT(Rf_coerceVector(Ain, REALSXP));
    } else {
	A = PROTECT(Rf_allocMatrix(REALSXP, m, n));
	Memcpy(REAL(A), REAL(Ain), (size_t)m * n);
    }

    SEXP jpvt = PROTECT(Rf_allocVector(INTSXP, n));
    for (int i = 0; i < n; i++) INTEGER(jpvt)[i] = 0;
    SEXP tau = PROTECT(Rf_allocVector(REALSXP, m < n ? m : n));
    int info, lwork = -1;
    double tmp;
    F77_CALL(dgeqp3)(&m, &n, REAL(A), &m, INTEGER(jpvt), REAL(tau),
		     &tmp, &lwork, &info);
    if (info < 0)
	Rf_error(_("error code %d from Lapack routine '%s'"), info, "dgeqp3");
    lwork = (int) tmp;
    double *work = (double *) R_alloc(lwork, sizeof(double));
    F77_CALL(dgeqp3)(&m, &n, REAL(A), &m, INTEGER(jpvt), REAL(tau),
		     work, &lwork, &info);
...
{
    if (!Rf_isMatrix(Bin)) Rf_error(_("'b' must be a numeric matrix"));

    SEXP B = PROTECT(Rf_isReal(Bin) ? Rf_duplicate(Bin) : Rf_coerceVector(Bin, REALSXP)),
	...
	*Bdims = INTEGER(Rf_coerceVector(Rf_getAttrib(B,  R_DimSymbol), INTSXP));
    ...
    F77_CALL(dormqr)("L", "T", &n, &nrhs, &k,
		     ...
		     &tmp, &lwork, &info);
    if (info != 0)
	Rf_error(_("error code %d from Lapack routine '%s'"), info, "dormqr [tmp]");
    lwork = (int) tmp;
    double *work = (double *) R_alloc(lwork, sizeof(double));
    F77_CALL(dormqr)("L", "T", &n, &nrhs, &k,
		     REAL(qr), &n, REAL(tau), REAL(B), &n,
		     work, &lwork, &info);
    if (info != 0)
	Rf_error(_("error code %d from Lapack routine '%s'"), info, "dormqr [work]");
    F77_CALL(dtrtrs)("U", "N", "N", &k, &nrhs,
		     REAL(qr), &n, REAL(B), &n, &info);
    if (info != 0)
	Rf_error(_("error code %d from Lapack routine '%s'"), info, "dtrtrs");
    UNPROTECT(1);
    return B;
}
...
static SEXP qr_qy_real(SEXP Q, SEXP Bin, SEXP trans)
{
    ...
    B = PROTECT(Rf_isReal(Bin) ? Rf_duplicate(Bin) : Rf_coerceVector(Bin, REALSXP));
    n = INTEGER(Rf_coerceVector(Rf_getAttrib(qr, R_DimSymbol), INTSXP))[0];
    Bdims = INTEGER(Rf_coerceVector(Rf_getAttrib(B, R_DimSymbol), INTSXP));
    if(Bdims[0] != n)
	Rf_error(_("right-hand side should have %d not %d rows"), n, Bdims[0]);
    ...
    F77_CALL(dormqr)("L", tr ? "T" : "N", &n, &nrhs, &k,
		     ...
		     &tmp, &lwork, &info);
    if (info != 0)
	Rf_error(_("error code %d from Lapack routine '%s'"), info, "dormqr");
    lwork = (int) tmp;
    work = (double *) R_alloc(lwork, sizeof(double));
    F77_CALL(dormqr)("L", tr ? "T" : "N", &n, &nrhs, &k,
		     REAL(qr), &n, REAL(tau), REAL(B), &n,
...
{
    int info, sign = 1, useLog = Rf_asLogical(logarithm);
    double modulus = 0.0; /* -Wall */

    if (!Rf_isMatrix(Ain)) Rf_error(_("'a' must be a numeric matrix"));
    if (useLog == NA_LOGICAL) Rf_error(_("argument 'logarithm' must be logical"));
    SEXP A = PROTECT(Rf_isReal(Ain) ? Rf_duplicate(Ain): Rf_coerceVector(Ain, REALSXP));
    int *Adims = INTEGER(Rf_coerceVector(Rf_getAttrib(Ain, R_DimSymbol), INTSXP));
    int n = Adims[0];
    if (Adims[1] != n) Rf_error(_("'a' must be a square matrix"));
	...
	/*Rf_warning("Lapack dgetrf(): singular matrix: U[%d,%d]=0", info,info);*/
	modulus =
    ...
    SEXP val = PROTECT(Rf_allocVector(VECSXP, 2));
    SEXP nm = PROTECT(Rf_allocVector(STRSXP, 2));
    SET_STRING_ELT(nm, 0, Rf_mkChar("modulus"));
    SET_STRING_ELT(nm, 1, Rf_mkChar("sign"));
    Rf_setAttrib(val, R_NamesSymbol, nm);
    SET_VECTOR_ELT(val, 0, Rf_ScalarReal(modulus));
    SEXP s_logarithm = Rf_install("logarithm");
    Rf_setAttrib(VECTOR_ELT(val, 0), s_logarithm, Rf_ScalarLogical(useLog));
    SET_VECTOR_ELT(val, 1, Rf_ScalarInteger(sign));
    Rf_setAttrib(val, R_ClassSymbol, Rf_ScalarString(Rf_mkChar("det")));
    UNPROTECT(3);
    return val;
}
...
#endif
	ans = Rf_mkString(""); /* LAPACK library not known */
	break;
    }
