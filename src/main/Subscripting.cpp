...
void Subscripting::Indices::applyNewNames(VectorBase *v) const
{
	...
	GCStackRoot<StringVector> newnames;
	{
		...
		if (nmattr)
			...
			newnames = StringVector::create(v->size());
	}
...
void Subscripting::Indices::initialize(const RObject *subscripts,
									   ...
									   std::size_t range_size)
{
	...
	if (!anyneg)
	{
		...
		for (std::size_t iin = 0; iin < rawsize; ++iin)
		{
			...
			if (index != 0.0)
				(*this)[iout++] = (std::isfinite(index) ? std::size_t(index) : 0);
		}
		m_min_lhssize = m_max_index;
	}
	else
	...
	for (std::size_t iraw = 0; iraw < rawsize; ++iraw)
	{
		...
		else
		{
			...
			{ // No
				...
				{ // The supplied subscript is a new name:
					if (!m_use_names)
											   ...
											   const VectorBase *v, const PairList *subscripts)
{
	...
	for (std::size_t d = 0; d < ndims; ++d)
	{
		...
		const StringVector *names = (dimnames ? SEXP_downcast<StringVector *>((*dimnames)[d].get()) : nullptr);
											...
											const IntVector *source_dims, const std::vector<Indices> &indicesvec)
{
	...
	for (std::size_t d = 1; d < ndims; ++d)
		(*dimindexers)[d].stride = (*dimindexers)[d - 1].stride * std::size_t((*source_dims)[d - 1]);
	return resultsize;
}
...
bool Subscripting::dropDimensions(VectorBase *v)
{
	...
	if (ngooddims > 1)
	{
		...
		if (havenames)
		{
			...
			StringVector *dimnamesnames = const_cast<StringVector *>(dimnames->names());
	...
	// Dimnames:
	{
		...
		if (dimnames)
		{
			...
			for (std::size_t d = 0; d < ndims; ++d)
			{
				...
				if (indices.size() > 0)
				{
					const StringVector *sv = static_cast<const StringVector *>((*dimnames)[d].get());
					if (sv)
