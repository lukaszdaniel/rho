...
void rho::AllocatorSuperblock::allocateArena()
{
  ...
  arena_superblock_end = arena_superblock_start + num_superblock * s_small_superblock_size;
...
{
  assert(block_size >= 32 && block_size <= 256 && "Only use allocateBlock() to allocate objects between 32 and 256 bytes.");
  assert((block_size & 7) == 0 && "The size argument must be a multiple of 8 bytes");
...
{
  assert(size_log2 >= 6 && size_log2 < GCNodeAllocator::s_num_medium_pools && "Can not allocate objects smaller than 64 bytes using allocateLarge()");
