 ...
 * merge = (n-1) x 2  matrix, described in help(hclust) */
    ...
    PROTECT(merge = Rf_coerceVector(merge, INTSXP)); i_merge = INTEGER(merge);
    PROTECT(which = Rf_coerceVector(which, INTSXP)); i_which = INTEGER(which);
    ...
    PROTECT(ans = Rf_allocMatrix(INTSXP, n, LENGTH(which)));
    i_ans = INTEGER(ans);
