	...
	namespace
	{
		...
		struct LocalVariableVisitor : public GCNode::const_visitor
		{
			...
			void visit(const GCNode *node)
			{
				...
				if (function == Symbol::obtain("<-") || function == Symbol::obtain("=") || function == Symbol::obtain("for"))
				{
					if (!expression->tail())
					{
					...
					const Symbol *local_var = dynamic_cast<const Symbol *>(expression->tail()->car());
					...
					if (local_var && (std::find(m_locals->begin(), m_locals->end(), local_var) == m_locals->end()))
					{
