 ...
 */
     ...
     */
        VectorBase(const VectorBase &pattern)
            : RObject(pattern), m_xtruelength(pattern.m_xtruelength), m_size(pattern.m_size)
        {
        }
     ...
     * vector.
     */
        static PairList *resizeAttributes(
            const PairList *attributes, size_type new_size);
     ...
     */
        static const char *staticTypeName() { return "(vector type)"; }
