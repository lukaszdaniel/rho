 ...
 */
    ...
    public:
        Promise(const RObject *valgen, Environment *env)
            : RObject(PROMSXP), m_data(valgen, env)
        {
        }

        Promise(PromiseData &&value)
            : RObject(PROMSXP), m_data(std::forward<PromiseData>(value))
        {
        }
     ...
     * @return true iff ... well, read the code!
     */
        bool isMissingSymbol() const { return m_data.isMissingSymbol(); }
     ...
     * evaluated.
     */
        Environment *environment() const { return m_data.m_environment; }
     ...
     */
        RObject *value() { return m_data.m_value; }
     ...
     * the Promise.
     */
        const RObject *valueGenerator() const { return m_data.m_valgen; }
     ...
     * @param val Value to be associated with the Promise.
     */
        void setValue(RObject *val) { m_data.setValue(val); }
 ...
 */
    ...
    inline PromiseData *PromiseData::getThis()
    {
        return m_is_pointer_to_promise
                   ? &static_cast<Promise *>(m_value.get())->m_data
                   : this;
    }
    ...
    {
        return m_is_pointer_to_promise
                   ? &static_cast<const Promise *>(m_value.get())->m_data
                   : this;
    }
