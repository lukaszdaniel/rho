...
static void kill_and_detach_child_ci(child_info_t *ci, int sig)
{
	...
	Rf_warning(_("unable to terminate child process: %s"), strerror(errno));
...
SEXP mc_cleanup(SEXP sKill, SEXP sDetach, SEXP sShutdown)
{
    ...
    if (Rf_isLogical(sKill)) {
	int lkill = Rf_asLogical(sKill);
	if (lkill == TRUE) sig = SIGTERM;
	else if (lkill == FALSE) sig = 0;
    } else {
	int ikill = Rf_asInteger(sKill);
	if (ikill >= 1 && ikill != NA_INTEGER)
	    sig = ikill;
    }
    if (sig == -1)
	Rf_error(_("invalid '%s' argument"), "mc.cleanup");

    int detach = Rf_asLogical(sDetach);
    if (detach == NA_LOGICAL)
	Rf_error(_("invalid '%s' argument"), "detach");

    int shutdown = Rf_asLogical(sShutdown);
    if (shutdown == NA_LOGICAL)
	Rf_error(_("invalid '%s' argument"), "shutdown");
...
SEXP mc_select_children(SEXP sTimeout, SEXP sWhich) 
{
	...
	if (!ci->detached && ci->ppid == ppid) {
		...
		if (!found)
		    ...
		    Rf_warning(_("cannot wait for child %d as it does not exist"), ci->pid);
	    } else {
	...
	for(;;) {
		...
		break;
	    if (timeout > 0) {
		remains = timeout - (Rf_currentTime() - before);
		if (remains <= 0)
    ...
    if (sr < 1) return Rf_ScalarLogical(TRUE); /* TRUE on timeout */
    ci = children;
#ifdef MC_DEBUG
    Dprintf(" - read select %d children: ", sr);
#endif
    res = Rf_allocVector(INTSXP, sr);
    res_i = INTEGER(res);
...
static SEXP read_child_ci(child_info_t *ci) 
{
	...
	{
	    SEXP pa;
	    PROTECT(pa = Rf_ScalarInteger(ci->pid));
...
{
    int pid = Rf_asInteger(sPid);
    return Rf_ScalarLogical(rm_child(pid));
}
