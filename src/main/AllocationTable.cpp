...
{
  assert(pointer != Allocation::s_empty_key && pointer != Allocation::s_deleted_key && "Trying to remove invalid key from alloctable.");
...
{
  assert(new_alloctable_bits > m_num_bits && "can not shrink this table");
