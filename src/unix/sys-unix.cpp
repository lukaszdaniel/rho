...
static void warn_status(const char *cmd, int res)
{
	...
	Rf_warning(_("running command '%s' had status %d and error message '%s'"),
		cmd, res, strerror(errno));
    else
	Rf_warning(_("running command '%s' had status %d"), cmd, res);
}
...
SEXP attribute_hidden do_system(SEXP call, SEXP op, SEXP args, SEXP rho)
{
	...
	if (timeout && tost.timedout) {
	    res = 124;
	    Rf_warning(_("command '%s' timed out after %ds"), cmd, timeout);
	} else
	    warn_status(cmd, res);
	...
	if (res == 127) 
	    Rf_warning(_("error in running command"));
	if (timeout && tost.timedout) {
	    res = 124;
	    Rf_warning(_("command '%s' timed out after %ds"), cmd, timeout);
	} 
