...
if (file.access("/dev/full", mode = 2) == 0) { # Not on all systems...
    ...
    # TODO Rho: Rho fails to catch warning message. Don't know why.
    # R revs: 72891 and 72898
    if(FALSE) stopifnot(inherits(tryCatch(write.table(data.frame(x=1),
                                                file = "/dev/full"),
                                    warning = identity),
                       "warning"))
}
...
# R-devel rev 73111
# TODO Rho: returnValue not in line with CR
...
stopifnot(exprs = {
    identical(res, 3)
#    identical(mret, 3) # TODO Rho: mret equals 29, not 3
#    identical(hret, 3) # TODO Rho: hret equals 29, not 3
    identical(lret, 29)
    identical(uvarg, 1)
#    identical(uvret, 3) # TODO Rho: uvret equals 29, not 3
})
##
## returnValue: callCC
fret <- NULL
...
stopifnot(exprs = {
    identical(res, 3)
    identical(fret, 31)
#    identical(mycallCCret, 3) # TODO Rho: mycallCCret equals 31, not 3
    identical(funret, 31)
})
...
if(FALSE) { # Rho doesn't user compiler package
