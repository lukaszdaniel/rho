...
SEXP do_setGraphicsEventEnv(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if(!gdd) Rf_errorcall(call, _("invalid device"));
    dd = gdd->dev;
    args=CDR(args);
	...
	!dd->canGenKeybd &&
	!dd->canGenIdle)
	Rf_error(_("this graphics device does not support event handling"));
...
SEXP do_getGraphicsEvent(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    prompt = CAR(args);
    if (!Rf_isString(prompt) || !Rf_length(prompt)) Rf_error(_("invalid prompt"));
		  ...
		  int buttons, double x, double y)
{
    ...
    PROTECT(handler = Rf_findVar(Rf_install(mouseHandlers[event]), dd->eventEnv));
	...
	PROTECT(bvec = Rf_allocVector(INTSXP, len));
	i = 0;
	if (buttons & leftButton) INTEGER(bvec)[i++] = 0;
	if (buttons & middleButton) INTEGER(bvec)[i++] = 1;
	if (buttons & rightButton) INTEGER(bvec)[i++] = 2;

	PROTECT(sx = Rf_ScalarReal( (x - dd->left) / (dd->right - dd->left) ));
	PROTECT(sy = Rf_ScalarReal((y - dd->bottom) / (dd->top - dd->bottom) ));
	PROTECT(temp = Rf_lang4(handler, bvec, sx, sy));
	     ...
	     const char *keyname)
{
    ...
    PROTECT(handler = Rf_findVar(Rf_install(keybdHandler), dd->eventEnv));
    if (TYPEOF(handler) == PROMSXP) {
	handler = Rf_eval(handler, dd->eventEnv);
	UNPROTECT(1); /* handler */
	PROTECT(handler);
...
void doIdle(pDevDesc dd)
{
    ...
    PROTECT(handler = Rf_findVar(Rf_install(idleHandler), dd->eventEnv));
    if (TYPEOF(handler) == PROMSXP) {
	handler = Rf_eval(handler, dd->eventEnv);
	UNPROTECT(1); /* handler */
	PROTECT(handler);
