...
static SEXP removeAttrib(SEXP vec, SEXP name)
{
    ...
    if (name == R_NamesSymbol && Rf_isPairList(vec)) {
...
{
    return Rf_getAttrib(x_, R_CommentSymbol);
}
...
static SEXP cache_class(const char *class_str, SEXP klass)
{
	...
	R_S4_extends_table = R_NewHashedEnv(R_NilValue, Rf_ScalarInteger(0));
	R_PreserveObject(R_S4_extends_table);
    }
    if(Rf_isNull(klass)) { /* retrieve cached value */
	SEXP val = Rf_findVarInFrame(R_S4_extends_table, Rf_install(class_str));
...
SEXP attribute_hidden do_namesgets(/*const*/ Expression* call, const BuiltInFunction* op, RObject* x_, RObject* value_)
{
    ...
    if (names == R_NilValue &&
	Rf_getAttrib(object, R_NamesSymbol) == R_NilValue)
	return object;
...
SEXP Rf_dimnamesgets(SEXP vec, SEXP val)
{
    ...
    dims = Rf_getAttrib(vec, R_DimSymbol);
...
SEXP attribute_hidden do_levelsgets(/*const*/ Expression* call, const BuiltInFunction* op, RObject* x_, RObject* value_)
{
    ...
    if (MAYBE_SHARED(object)) object = Rf_duplicate(object);
    Rf_setAttrib(object, R_LevelsSymbol, levels);
    return object;
}
...
/* "dim" and "dimnames" are set that the "dim" is attached first. */
    ...
    if (nattrs > 0) {
	names = Rf_getAttrib(attrs, R_NamesSymbol);
	if (names == R_NilValue)
    ...
    if (Rf_isList(object))
	Rf_setAttrib(object, R_NamesSymbol, R_NilValue);
	...
	for (i = 0; i < nattrs; i++) {
		...
		i0 = i;
		Rf_setAttrib(object, R_DimSymbol, VECTOR_ELT(attrs, i));
		break;
	    }
	}
	for (i = 0; i < nattrs; i++) {
...
static void check_slot_assign(SEXP obj, SEXP input, SEXP value, SEXP env)
{
	...
	SEXP e = PROTECT(Rf_lang1(Rf_install("initMethodDispatch")));
    ...
    SEXP e = PROTECT(Rf_lang4(checkAt, objClass, input, valueClass));
    Rf_eval(e, env);
    UNPROTECT(3);
}
		       ...
		       const char **rn, const char **cn)
{
	...
	*cl = VECTOR_ELT(dimnames, 1);
	nn = Rf_getAttrib(dimnames, R_NamesSymbol);
	if (Rf_isNull(nn)) {
	    *rn = nullptr;
	    *cn = nullptr;
	}
...
{
    return Rf_getAttrib(x, R_DimNamesSymbol);
}
...
#define R_SLOT_INIT							\
	...
	return(1);
    /* else */
    return(Rf_getAttrib(obj, name) != R_NilValue);
}
...
SEXP R_do_slot(SEXP obj, SEXP name) {
	...
	if(value == R_NilValue) {
		...
		classString = Rf_getAttrib(obj, R_ClassSymbol);
...
R_getS4DataSlot(SEXP obj, SEXPTYPE type)
{
  ...
  if(TYPEOF(obj) != S4SXP || type == S4SXP) {
    ...
    if(s3class != R_NilValue) {/* replace class with S3 class */
      Rf_setAttrib(obj, R_ClassSymbol, s3class);
      Rf_setAttrib(obj, s_dot_S3Class, R_NilValue); /* not in the S3 class */
    }
