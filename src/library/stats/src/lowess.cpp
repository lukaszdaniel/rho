...
{
    if(TYPEOF(x) != REALSXP || TYPEOF(y) != REALSXP) Rf_error("invalid input");
    int nx = LENGTH(x);
    if (nx == NA_INTEGER || nx == 0) Rf_error("invalid input");
    double f = Rf_asReal(sf);
    if (!R_FINITE(f) || f <= 0) Rf_error(_("'f' must be finite and > 0"));
    int iter = Rf_asInteger(siter);
    if (iter == NA_INTEGER || iter < 0) 
	Rf_error(_("'iter' must be finite and >= 0"));
    double delta = Rf_asReal(sdelta), *rw, *res;
    if (!R_FINITE(delta) || delta < 0) 
	Rf_error(_("'delta' must be finite and > 0"));
    SEXP ans;
    PROTECT(ans = Rf_allocVector(REALSXP, nx));
