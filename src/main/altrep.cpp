...
ALTREP_UNSERIALIZE_EX(SEXP info, SEXP state, SEXP attr, int objf, int levs)
{
	...
	case VECSXP:
	case EXPRSXP:
	    Rf_warning("cannot unserialize ALTVEC object of class '%s' from "
    ...
    if (type != rtype)
	Rf_warning("serialized class '%s' from package '%s' has type %s; "
...
{
    Rf_error("unknown ALTLOGICAL type"); /* dispatch here */
}

Rcomplex attribute_hidden ALTCOMPLEX_ELT(SEXP x, R_xlen_t i)
{
    Rf_error("unknown ALTCOMPLEX type"); /* dispatch here */
}

Rbyte attribute_hidden ALTRAW_ELT(SEXP x, R_xlen_t i)
{
    Rf_error("unknown ALTRAW type"); /* dispatch here */
}
...
static SEXP new_compact_intseq(R_xlen_t n, int n1, int inc)
{
    ...
    SEXP info = Rf_allocVector(INTSXP, 3);
