...
template <typename T, SEXPTYPE ST>
rho::FixedVector<T, ST>::FixedVector(const FixedVector<T, ST> &pattern)
    : VectorBase(pattern), m_data(reinterpret_cast<T *>(m_first_element_storage))
{
    constructElementsIfNeeded();
...
template <typename T, SEXPTYPE ST>
rho::FixedVector<T, ST> *rho::FixedVector<T, ST>::create(size_type sz)
{
...
template <typename T, SEXPTYPE ST>
void rho::FixedVector<T, ST>::constructElements(iterator from, iterator to)
{
...
template <typename T, SEXPTYPE ST>
void rho::FixedVector<T, ST>::destructElements(iterator from, iterator to)
{
    for (iterator p = from; p != to; ++p)
        p->~T();
}
...
template <typename T, SEXPTYPE ST>
void rho::FixedVector<T, ST>::visitElements(
